...
function BottomNav({ onTransport, onPacking, onHome }: BottomNavProps) {
  const items = [
    { key: 'transport', Icon: Truck, label: 'הובלות', onClick: onTransport },
    { key: 'packing', Icon: Package, label: 'אריזה', onClick: onPacking },
    { key: 'home', Icon: Home, label: 'תפריט', onClick: onHome },
  ]
    ...
    >
      <Box
        ...
        sx={{
          ...
          py: '15px',
          borderRadius: '999px',
          backgroundColor: active ? 'rgba(90, 50, 18, 0.92)' : 'rgba(90, 50, 18, 0.38)',
      ...
      >
        {busy ? <CircularProgress size={20} sx={{ color: 'rgba(255,255,255,0.8)' }} /> : label}
      </Box>
    </Box>
  )
}
...
function ShipmentList({ shipments, loading, error, onSelect, onBack, nav }: ShipmentListProps) {
  return (
...
function PackingUnitCard({ unit, selected, locked, onToggle }: PackingUnitCardProps) {
  ...
  const sourceLines = unit.sourceDescription
    ? unit.sourceDescription.split('|').map((part) => part.trim()).filter(Boolean)
    : []
  ...
  return (
    <Box
      ...
      sx={{
        backgroundColor: selected ? 'rgba(101, 55, 12, 0.80)' : 'rgba(238, 205, 135, 0.72)',
        backdropFilter: 'blur(10px)',
        WebkitBackdropFilter: 'blur(10px)',
        borderRadius: '16px',
        border: selected
    ...
    >
      ...
      <Box sx={{ flex: 1, p: '12px 6px 12px 14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
          <Box
            sx={{
              ...
              px: '12px',
              py: '2px',
              backgroundColor: selected ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.45)',
            }}
          >
            <Typography
              sx={{
...
function PackingUnitList({
  ...
  const allSelected = selectable.length > 0 && selectable.every((unit) => selected.has(unit.id))
  ...
  return (
    ...
    >
      ...
      {incomplete && (
        <Box sx={{ position: 'relative', zIndex: 2, px: '5vw', pb: '8px', flexShrink: 0 }}>
          <Box
            sx={{
...
function CompletionDialog({ open, onConfirm }: { open: boolean; onConfirm: () => void }) {
  return (
    <Dialog
      open={open}
      slotProps={{
...
export default function ReceivingPage({ userId, onExit, onNavigate }: ReceivingPageProps) {
  ...
  const submit = async () => {
    ...
    setSubmitError(null)
    try {
      const result = await receivePackingUnits(shipment.id, [...selected], userId)
      setSelected(new Set())
      if (result.finalized) {
        setCompleted(true)
        return
