...
import {
  fetchEligiblePackingUnits,
  createShipment,
  type EligiblePackingUnit,
} from "../lib/api";
...
function Step1({
  ...
  const [transportType, setTransportType] = useState<"truck" | "other">(
    initialType,
  );
  ...
  const canProceed =
    isValidPlate &&
    (transportType !== "other" || otherDescription.trim().length > 0);
...
function Step2({
  ...
  const selectedUnitCount = selectedCrates.reduce(
  ...
  const handleFinish = async () => {
    ...
    if (userId && orgScopeId) {
      try {
        const result = await createShipment(
          {
            ...
            transportDescription:
              transportType === "other" ? otherDescription : undefined,
        ...
        console.warn(
          "createShipment API call failed, proceeding with local summary:",
          err,
        );
      }
    }
    ...
    >
      ...
      >
        ...
        {filtered.map((crate) => {
          const dest = (() => {
            try {
              return crate.destinationDescription
                ? JSON.parse(crate.destinationDescription)
                : null;
            } catch {
              return null;
            }
          })();
          const srcLines = crate.sourceDescription
            ? crate.sourceDescription.split("|").map((s) => s.trim())
            : [];
          const col = crate.checked ? "#fff" : "#2d1b0a";
          const sub = crate.checked ? "rgba(255,255,255,0.75)" : "#6e4530";
          const borderCol = crate.checked
          ...
          return (
            <Box
              ...
              sx={{
                backgroundColor: crate.checked
                  ? "rgba(101, 55, 12, 0.80)"
                  : "rgba(238, 205, 135, 0.72)",
                backdropFilter: "blur(10px)",
                WebkitBackdropFilter: "blur(10px)",
                borderRadius: "16px",
                border: crate.checked
            ...
            >
              ...
              >
                <Package
                  size={38}
                  color={crate.checked ? "rgba(255,255,255,0.6)" : "#9B6E3C"}
                  strokeWidth={1.2}
                />
              </Box>
          ...
          sx={{
            ...
            backgroundColor:
              anyChecked && !submitting
                ...
                : "rgba(90, 50, 18, 0.38)",
            color:
              anyChecked && !submitting ? "white" : "rgba(255,255,255,0.5)",
            fontFamily: "Heebo, sans-serif",
            fontWeight: 700,
            fontSize: "1.05rem",
            textAlign: "center",
            ...
            boxShadow:
              anyChecked && !submitting ? "0 4px 18px rgba(0,0,0,0.3)" : "none",
            backdropFilter: "blur(4px)",
            WebkitBackdropFilter: "blur(4px)",
            letterSpacing: 0.5,
            WebkitTapHighlightColor: "transparent",
            ...
            gap: 1,
            ...(anyChecked &&
              !submitting && { "&:active": { transform: "scale(0.97)" } }),
          }}
        >
          {submitting ? (
            <CircularProgress
