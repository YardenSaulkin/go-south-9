...
React/Vite (Hebrew RTL)
  └── HTTP /api + x-user-id (temporary demo context)
       └── NestJS controllers
...
- `App.tsx` owns navigation and the temporary user selector.
- `PackingUnitPage.tsx` implements the focused source → type → items → destination → completion flow.
- `api.ts` is the HTTP entry point for this flow and sends the selected demo user's server-resolved identity.
...
- Current mode: temporary demo selector backed by `users`.
- Server-visible identity: `x-user-id`, validated as UUID and resolved from the DB.
...
- Demo identity is explicitly not production authentication.
